<?php
 include("connect.php");
  include("header.php");


  if(isset($_GET['category']))
{
	$a = $_GET['category'];	
	$query_pro = "SELECT * FROM product WHERE category = $a";
}
else
{

	$query_pro = "SELECT * FROM product";
}



$result_pro = mysqli_query($con, $query_pro);


?>
			<div id="right-content-bottom">
				<h3>New Products</h3>
				<?php
				while($data_pro = mysqli_fetch_assoc($result_pro))
				{
					$discount = $data_pro['discount'];
					$price = $data_pro['product_price'];
					$x = $price*$discount/100;
					$new_price=$price-$x;
				?>
?>
			
			
				<div class="right-content-block">

					<div class="right-content-subblock">
					<div id="right-block1">
						<!-- <img src="upload/<?php echo $data_pro['product_image']; ?>" height="150" width="130" /> -->
						 <img src="image/2.jpg" height="150" width="130" style="border-radius: 10px" /> 
					</div>
					<div id="right-block2">
						<p><?php echo $data_pro['product_name']; ?></p>
						<p class="old_price">$ <?php echo $data_pro['product_price']; ?></p>
						<p class="new_price">$ <?php echo $new_price; ?>.00</p>
						<a href="view_more.php?pid=<?php echo $data_pro['id']; ?>" class="view-more-btn">View</a>
						<a href="add_to_cart.php?pid=<?php echo $data_pro['id']; ?>" class="view-more-btn">Add To Cart</a>
					</div>	
					</div>
					<div class="right-content-subblock">
						<div id="right-block3">
							<img src="upload/6.jpg" height="150" width="130" style="border-radius: 10px" />
						</div>
						<div id="right-block4">
							<p><?php echo $data_pro['product_name']; ?></p>
						<p class="old_price">$ <?php echo $data_pro['product_price']; ?></p>
						<p class="new_price">$ <?php echo $new_price; ?>.00</p>
						<a href="view_more.php?pid=<?php echo $data_pro['id']; ?>" class="view-more-btn">View</a>
						<a href="add_to_cart.php?pid=<?php echo $data_pro['id']; ?>" class="view-more-btn">Add To Cart</a>
						</div>
					</div>
					<div class="right-content-subblock">
						<div id="right-block5">
							<img src="upload/3.jpg" height="145" width="120" style="border-radius: 10px" />
						</div>
						<div id="right-block6">
							<p><?php echo $data_pro['product_name']; ?></p>
						<p class="old_price">$ <?php echo $data_pro['product_price']; ?></p>
						<p class="new_price">$ <?php echo $new_price; ?>.00</p>
						<a href="view_more.php?pid=<?php echo $data_pro['id']; ?>" class="view-more-btn">View</a>
						<a href="add_to_cart.php?pid=<?php echo $data_pro['id']; ?>" class="view-more-btn">Add To Cart</a>
						</div>
					</div>
					<div class="right-content-subblock">
						<div id="right-block7">
							<img src="image/vaio.jpg" height="145" width="130" style="border-radius: 10px" />
						</div>
						<div id="right-block8">
							<p><?php echo $data_pro['product_name']; ?></p>
						<p class="old_price">$ <?php echo $data_pro['product_price']; ?></p>
						<p class="new_price">$ <?php echo $new_price; ?>.00</p>
						<a href="view_more.php?pid=<?php echo $data_pro['id']; ?>" class="view-more-btn">View</a>
						<a href="add_to_cart.php?pid=<?php echo $data_pro['id']; ?>" class="view-more-btn">Add To Cart</a>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
<?php
 include("footer.php");


?>